...
class BaseService():
    ...
    def do_screenShot(self, fileName: str, folderPath: str, url: str):
        ...
        self._picNameArray = []
        self._shareFolderPath = folderPath #f'Z:\\02-共用資料區(3G)\\03-戰情\\3F_Bonding\\Daily'
        self._url = url
        ...
        if _nowTime.strftime('%H') == '00':
            _removeFolder = (_nowTime + datetime.timedelta(days=-3)
                             ).strftime('%Y%m%d')
        ...
        # 3F Bonding
        driver.get("http://10.132.133.164/Function/ENG1/Bonding_Output.aspx")
