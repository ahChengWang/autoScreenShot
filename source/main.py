...
def do_screenShot():
    ...
    print(_nowTime.strftime('%Y-%m-%d %H:%M:%S'))

    # 刪除三天前資料夾
    if _nowTime.strftime('%H') == '00':
        _removeFolder = (_nowTime + datetime.timedelta(days=-3)).strftime('%Y%m%d')
        _dirList = os.listdir(_shareFolderPath)
    
        for dirName in _dirList:
            if int(dirName) <= int(_removeFolder):
                shutil.rmtree(f'{_shareFolderPath}\{dirName}')
    ...
    for i in range(1, 4):
        ...
        if(os.path.exists(f'.\{_picName}')):
            shutil.move(f'.\{_picName}',
                        f'{_shareFolderPath}\{_strDate}\{_picName}')
...
schedule.every().days.at('08:30').do(do_screenShot)
schedule.every().days.at('10:30').do(do_screenShot)
schedule.every().days.at('12:30').do(do_screenShot)
schedule.every().days.at('14:30').do(do_screenShot)
schedule.every().days.at('16:30').do(do_screenShot)
schedule.every().days.at('18:30').do(do_screenShot)
schedule.every().days.at('20:30').do(do_screenShot)
schedule.every().days.at('22:30').do(do_screenShot)
schedule.every().days.at('00:30').do(do_screenShot)
schedule.every().days.at('02:30').do(do_screenShot)
schedule.every().days.at('04:30').do(do_screenShot)
schedule.every().days.at('06:30').do(do_screenShot)
